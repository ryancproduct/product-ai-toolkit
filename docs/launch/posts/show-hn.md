...
The kit includes 15 commands (like `/debate`, `/experiment`, `/impact-sizing`), 14+ reusable skills, and 9 specialist agents. Each agent has domain-specific prompting, tool access, and quality gates. For example, `/debate` spawns Champion and Sceptic agents that argue for and against a feature idea using real analytics data, then synthesises into a risk register and go/no-go recommendation.
